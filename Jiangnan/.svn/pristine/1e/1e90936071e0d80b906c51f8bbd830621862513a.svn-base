    ...
    public abstract class MonoSingleton<T> : MonoBehaviour, ISingleton where T : MonoSingleton<T>
    {
        ...
        public static T Instance
        {
            get
            {
                if (instance == null)
                {
                    // 1. 尝试在场景中寻找（包括非激活物体，如果是 Unity 6000+）
#if UNITY_6000
                    ...
                    if (instance == null)
                    {
                        // 2. 核心逻辑：确保有一个统一的管理组根节点
                        GameObject root = GameObject.Find("[Global_Managers]");
                        if (root == null)
                        {
                            root = new GameObject("[Global_Managers]");
                            DontDestroyOnLoad(root);
                        }

                        // 3. 在根节点下创建单例物体
                        GameObject obj = new GameObject(typeof(T).Name);
                        obj.transform.SetParent(root.transform); // 将其设为管理组的子物体
